...
class Stores extends Admin_Controller 
{
	...
	public function fetchCategoryData()
	{
		...
		foreach ($data as $key => $value) {
			...
			$type = ($value['type'] == 1) ? '<span class="label label-info">Colony</span>' : '<span class="label label-info">Store</span>';

			if(in_array('updateStore', $this->permission)) {
				$buttons = '<button type="button" class="btn btn-default" style="border: 2px solid #4CAF50" onclick="editFunc('.$value['id'].')" data-toggle="modal" data-target="#editModal"><i class="fa fa-pencil" style="color:#4CAF50"></i></button>';
			}

			if(in_array('deleteStore', $this->permission)) {
				$buttons .= ' <button type="button" class="btn btn-default" style="border: 2px solid #f44336" onclick="removeFunc('.$value['id'].')" data-toggle="modal" data-target="#removeModal"><i class="fa fa-trash" style="color:#f44336"></i></button>';
			}
